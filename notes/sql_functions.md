...
## 12. DISTINCT 키워드(예악어)
- **설명:** 중복된 행을 제거하고 고유한 값만 출력해주는 키워드.
- **예시 코드:**
  ```sql
  SELECT COUNT(DISTINCT NAME)
  FROM ANIMAL_INS;
  -- 이름이 '몽이'인 개가 10마리여도 1종류로 계산합니다.
  ```

---

## 13. SUBSTRING(문자열, 시작위치, 길이)
- **설명:** 문자열의 일부분만 떼어내서 출력하는 함수
- **예시 코드:**
  ```sql
  SELECT SUBSTRING(주민번호, 8, 1) AS 성별코드
  FROM USERS;
  -- 결과: '1' 또는 '2' (여덟 번째 자리가 성별을 나타냄. 일곱 번째 자리는 '-'를 나타냄.)
  ```
